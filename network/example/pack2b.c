...
int main(void) {
  ...
  printf("'%c' %" PRId32 " %" PRId16 " %" PRId32 " \"%s\" %f\n", magic, ps2,
         monkeycount, altitude, s2, absurdityfactor);
  ...
  int32_t i, i2;
  int32_t test32[14] = {
      0,           -0,      1,           2,       -1, -2, INT_MAX >> 1,
      INT_MAX - 1, INT_MAX, INT_MIN + 1, INT_MIN, 0,  0,  0};

  int16_t j, j2;
  int16_t test16[14] = {
      0,        -0,           1,        2, -1, -2, SHRT_MAX >> 1, SHRT_MAX - 1,
      SHRT_MAX, SHRT_MIN + 1, SHRT_MIN, 0, 0,  0};
  ...
  if (sizeof(float32_t) != 4 || sizeof(float64_t) != 8) {
    ...
    if (f32 == NULL || f64 == NULL) {
      ...
      printf("Change the typedefs at the top of this source to the right "
             "types.\n");
      return 1;
    }

    printf("Please modify this source so the following typedefs are at the "
           "top:\n\n");
  ...
  {
    float64_t testf64[7] = {0.0,         1.0,     -1.0,   DBL_MIN * 2,
