...
void select_rvmessage(tcpsocketserver *t_server, FILE *file) {
  ...
  if (select(t_server->fdmax + 1, &t_server->read_fds, NULL, NULL, NULL) ==
    ...
    // server select error
    if (FILEORNOT)
      fprintf(file, "error in socket select!\n");
    else {
      perror("select");
      exit(4);
    }
    ...
    if (FD_ISSET(i, &t_server->read_fds)) { // we got one!!
      if (i == t_server->listener) {
        if (t_server->numexist >= MAXCONNECTION && MAXCONNECTION) {
          // if we reach the max connection, accept and close
          if (FILEORNOT)
            fprintf(file, "We have reach the max connection!\n");
          else
        ...
        if (t_server->nbytes == 0) {
          ...
          // connection closed
          if (FILEORNOT)
            fprintf(file, "selectserver: socket %d hung up\n", i);
          else
