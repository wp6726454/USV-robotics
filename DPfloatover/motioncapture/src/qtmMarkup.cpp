...
bool CMarkup::x_AddElem(LPCTSTR szName, LPCTSTR szValue, bool bInsert,
                        bool bAddChild) {
  if (bAddChild) {
...
std::string CMarkup::Format(const char* fmt, ...) {
  ...
  if (NULL != fmt) {
    ...
    size_t len = vsnprintf(NULL, 0, fmt, marker) + 1;

    // Create a char vector to hold the formatted string.
    vector<char> buffer(len, '\0');
    // int nWritten = _vsnprintf_s(&buffer[0], buffer.size(), len, fmt, marker);
    int nWritten = vsnprintf(&buffer[0], buffer.size(), fmt, marker);
