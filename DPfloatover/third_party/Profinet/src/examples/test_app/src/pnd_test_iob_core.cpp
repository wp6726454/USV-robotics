...
{ 1  /* CM_AR_REASON_1 */                           , "Sequence爊umbers燿o爊ot爉atch?no爈onger爑sed爄n爒ersions?=燰3.9)" }, \
        ...
        case PNIO_ALARM_DIAGNOSTIC:
            ...
            switch(pCbfPrm->AlarmInd.pAlarmData->AlarmAinfo.UserStrucIdent)
            {
                ...
                case 0x8002:    /* 0x8002 :  extended diagnosis alarm                    */
                    ...
                    printf("ChannelNumber       = 0x%04x\n", (pCbfPrm->AlarmInd.pAlarmData->AlarmAinfo.UAData.ext_diag.ChannelNumber));
                    printf("ChannelProperties   = 0x%04x\n", (pCbfPrm->AlarmInd.pAlarmData->AlarmAinfo.UAData.ext_diag.ChannelProperties));
                    printf("ChannelErrorType    = 0x%04x\n", (pCbfPrm->AlarmInd.pAlarmData->AlarmAinfo.UAData.ext_diag.ChannelErrorType));
                    printf("ExtChannelErrorType = 0x%04x\n", (pCbfPrm->AlarmInd.pAlarmData->AlarmAinfo.UAData.ext_diag.ExtChannelErrorType));
                    printf("ExtChannelAddValue  = 0x%08lx\n", (pCbfPrm->AlarmInd.pAlarmData->AlarmAinfo.UAData.ext_diag.ExtChannelAddValue));
...
void print_ium0(PNIO_IM0_TYPE *im0)
{
    ...
    printf("\t  ####################################################################\n");
    printf("\t  # BlockHeader->BlockType:         0x%02x                            \n", im0->BlockHeader.BlockType);
    printf("\t  # BlockHeader->BlockLength:       0x%02x                            \n", im0->BlockHeader.BlockLength);
    ...
    printf("\t  # VendorIDHigh:                0x%02x                            \n", im0->VendorIDHigh);
    printf("\t  # VendorIDLow:                 0x%02x                            \n", im0->VendorIDLow);
    printf("\t  # OrderID:                     %s                                \n", orderid);
    printf("\t  # Serial Number:               %s                                \n", serialnum);
    printf("\t  # Hardware_Revision:              %ld                               \n", SWAP_16(im0->IM_Hardware_Revision));
    printf("\t  # Software_Revision:              %c%d%d%d                          \n", (char)im0->IM_Software_Revision[0], im0->IM_Software_Revision[1], im0->IM_Software_Revision[2], im0->IM_Software_Revision[3]);
    printf("\t  # Revision_Counter:               %ld                               \n", SWAP_16(im0->IM_Revision_Counter));
    printf("\t  # Profile_ID:                     %ld                               \n", SWAP_16(im0->IM_Profile_ID));
    printf("\t  # Profile_Specific_Type:          %ld                               \n", SWAP_16(im0->IM_Profile_Specific_Type));
    printf("\t  # Version_Major:               %d                                \n", im0->IM_Version_Major);
    printf("\t  # Version_Minor:               %d                                \n", im0->IM_Version_Minor);
    printf("\t  # Supported:                      0x%02lx                           \n", SWAP_16(im0->IM_Supported));
    printf("\t  ####################################################################\n");
}
...
void print_ium1(PNIO_IM1_TYPE *im1)
{
    ...
    printf("\t  # Function Tag:                %s                                 \n", IM_Tag_Function);
    printf("\t  # Location Tag:                %s                                 \n", IM_Tag_Location);
    printf("\t  ####################################################################\n");
}
...
void callback_for_device_activation(PNIO_CBE_PRM * pCbfPrm)
{
    ...
    case PNIO_DA_TRUE:
        printf("Device with address %d has been activated with result %x \n" ,
            (int)pCbfPrm->DevActConf.pAddr->u.Addr, (unsigned int)pCbfPrm->DevActConf.Result);
        break;
    case PNIO_DA_FALSE:
...
void callback_for_diagnostics_request(PNIO_CBE_PRM * pCbfPrm)
{
	...
	switch(pCbfPrm->CtrlDiagConf.pDiagData->DiagService)
	{
	    case PNIO_CTRL_DIAG_CONFIG_SUBMODULE_LIST:
		{
			...
			pDiagConfSub = (PNIO_CTRL_DIAG_CONFIG_SUBMODULE *)pCbfPrm->CtrlDiagConf.pDiagDataBuffer;
					
			n_pnio_submodule_cnt = (pCbfPrm->CtrlDiagConf.DiagDataBufferLen) / sizeof (PNIO_CTRL_DIAG_CONFIG_SUBMODULE);

			printf("\n\n");
			printf("Read out submodule list ... (%3d submodules) bufferlen: %d size: %d \n\r", n_pnio_submodule_cnt, pCbfPrm->CtrlDiagConf.DiagDataBufferLen, sizeof(PNIO_CTRL_DIAG_CONFIG_SUBMODULE));
	    ...
	    case PNIO_CTRL_DIAG_DEVICE_DIAGNOSTIC:
		{
			...
			DiagDeviceState = (PNIO_CTRL_DIAG_DEVICE_DIAGNOSTIC_DATA *)pCbfPrm->CtrlDiagConf.pDiagDataBuffer;
			...
			switch(DiagDeviceState->ErrorCause)
			{
				...
				case 8: // CM_ERROR_CAUSE_IP_SET_ABORTED
					...
					printf("MAC Address             : %02x:%02x:%02x:%02x:%02x:%02x\n", 
							DiagDeviceState->AdditionalInfo[9], DiagDeviceState->AdditionalInfo[8]
						, DiagDeviceState->AdditionalInfo[7], DiagDeviceState->AdditionalInfo[6]
						, DiagDeviceState->AdditionalInfo[5], DiagDeviceState->AdditionalInfo[4]);
					break;
	    ...
	    case PNIO_CTRL_DIAG_CONFIG_NAME_ADDR_INFO:
		{
			...
			DiagConfigNameAddrInfo = (PNIO_CTRL_DIAG_CONFIG_NAME_ADDR_INFO_DATA *)pCbfPrm->CtrlDiagConf.pDiagDataBuffer;
...
void callback_for_ifc_rema_read( PNIO_CBE_PRM * pCbfPrm )
{
	...
	fwrite(pCbfPrm->RemaReadConf.RemaXMLBuffer,1,pCbfPrm->RemaReadConf.RemaXMLBufferLength,(FILE*)hXMLFile);
...
PNIO_UINT32 pnd_test_dev_activate(PNIO_DEV_ACT_TYPE activate_mode)
{
    ...
    // a logical address has no direction (don't care) but we set it to a valid value
            ...
            case 1:
                {
                    ...
                    fill_buffer_with_blank_char(im_data.IM_Tag_Function, sizeof(im_data.IM_Tag_Function));
                    ...
                    fill_buffer_with_blank_char(im_data.IM_Tag_Location, sizeof(im_data.IM_Tag_Location));

                    result = PNIO_rec_write_req(g_ApplHandle, &Addr, RecordIndex, ReqRef, Length, (PNIO_UINT8*) &im_data);
            ...
            case 2:
                {
                    ...
                    result = PNIO_rec_write_req(g_ApplHandle, &Addr, RecordIndex, ReqRef, 22, (PNIO_UINT8*) &im_data);
            ...
            case 3:
                {
                    ...
                    fill_buffer_with_blank_char(im_data.IM_Descriptor, sizeof(im_data.IM_Descriptor));
            ...
            case 4:
                {
                    ...
                    fill_buffer_with_blank_char(im_data.IM_Signature, sizeof(im_data.IM_Signature));
...
void pnd_test_controller_open(PNIO_DEBUG_SETTINGS_PTR_TYPE  DebugSettings)
{
    ...
    result = PNIO_controller_open(1, PNIO_CEP_MODE_CTRL,callback_for_ds_read_conf ,callback_for_ds_write_conf ,callback_for_alarm_ind , &g_ApplHandle);
    if (result != PNIO_OK)
    {
...
void pnd_test_trig_diag_req(void)
{
	...
	while (cmd != 4)
	{
		...
		result = PNIO_ctrl_diag_req(g_ApplHandle, pDiagReq); // TODO: return error values are defined in PGH_IO-Base_76.pdf, bu errrorlari da bildir.
...
void pnd_test_iosystem_reconfig(void)
{
		...
		case PNIO_IOS_RECONFIG_MODE_TAILOR: // Step-2
			{
				...
				printf("\n");
				printf("------------------------------------------------------------------------\n");
				printf("Please type the number of the optional IODevices in new configuration.    \n");
				printf("------------------------------------------------------------------------\n");
				...
				if( DeviceCnt != 0 )
				{
					...
					printf("\n");
					printf("----------------------------------------------------------------------------    \n");
					printf("Please type logical addresses of the optional IODevices in new configuration.     \n");
					printf("----------------------------------------------------------------------------    \n");
				...
				printf("\n");
				printf("------------------------------------------------------------------------\n");
				printf("Please type the number of entries in the PortInterconnectionList.         \n");
				printf("------------------------------------------------------------------------\n");
				...
				{
					PortInterconnectionList = (PNIO_ADDR *) malloc( 2 * PortInterconnectionCnt * sizeof(PNIO_ADDR) );
					...
					printf("\n");
					printf("------------------------------------------------------------------------\n");
					printf("Please type logical addresses of the PortInterconnectionList pairs.       \n");
					printf("------------------------------------------------------------------------\n");
				}
	...
	result = PNIO_iosystem_reconfig( g_ApplHandle, Mode, DeviceCnt, DeviceList, PortInterconnectionCnt, PortInterconnectionList );
	...
	if( Mode == PNIO_IOS_RECONFIG_MODE_TAILOR)
	{
		...
		if( PortInterconnectionList != PNIO_NULL )
			free(PortInterconnectionList);
	}
}

void pnd_test_register_setmode_cbf(void)
{
    PNIO_register_cbf( g_ApplHandle, PNIO_CBE_MODE_IND, callback_for_mode_change_indication);
}

void pnd_test_register_devact_cbf(void)
{
    PNIO_register_cbf( g_ApplHandle, PNIO_CBE_DEV_ACT_CONF, callback_for_device_activation);
}

void pnd_test_register_diag_req_cbf(void)
{
	PNIO_register_cbf( g_ApplHandle, PNIO_CBE_CTRL_DIAG_CONF, callback_for_diagnostics_request);
}

void pnd_test_register_iosystem_reconfig(void)
{
	PNIO_register_cbf( g_ApplHandle, PNIO_CBE_IOSYSTEM_RECONFIG, callback_for_iosystem_reconfig);
}        
...
void pnd_test_interface_open(PNIO_DEBUG_SETTINGS_PTR_TYPE  DebugSettings)
{
    ...
    result = PNIO_interface_open(1, callback_for_ifc_record_read_conf ,callback_for_alarm_ind , &g_ApplHandle);
    if (result != PNIO_OK)
    {
...
{
	PNIO_interface_register_cbf( g_ApplHandle, PNIO_CBE_IFC_SET_ADDR_CONF, callback_for_ifc_set_ip_and_nos);
}

void pnd_test_register_interface_rema_read(void)
{
	PNIO_interface_register_cbf( g_ApplHandle, PNIO_CBE_REMA_READ_CONF, callback_for_ifc_rema_read);
}
...
PNIO_UINT32 pnd_test_interface_data_read()
{
    ...
    while (cmd != 1)
    {
        ...
        switch(cmd)
        {
            case 0:
                {
                    result = PNIO_interface_rec_read_req(g_ApplHandle, &Addr, 0x0000B081, ReqRef, Length);
                    break;
                }
        }
...
void pnd_test_interface_set_ip_and_nos(void)
{
	...
	while (cmd != 4)
	{
        ...
        if(cmd == 1 || cmd == 3)
        {
			...
			printf("\n");
			printf("------------------------------------------------------------------------\n");
			printf("Please type IpAddress, Gateway, NetMask and Remanent state respectively.  \n");
			printf("------------------------------------------------------------------------\n");
			// be careful about scanf! %d -> decimal integer which is 4 bytes!
			printf("Ip Address     : ");
			scanf("%d%d%d%d",   &dump_address[0],
						        &dump_address[1],
								&dump_address[2],
								&dump_address[3] );
        ...
        result = PNIO_interface_set_ip_and_nos( g_ApplHandle, (PNIO_SET_IP_NOS_MODE_TYPE) Mode, IpV4, NoS );
