...
int Netinit() {
  ...
  for (i = 0; i < (int)ifNumber; i++) {
    ...
    MaxLnh = LSA_MAX(MaxLnh, nets[i]->n_lnh);
    MaxMtu = LSA_MAX(MaxMtu, nets[i]->n_mtu);
  ...
  for (i = 0; i < (int)ifNumber; i++) {
    if (nets[i]->n_init != NULL) /* If iface has init routine... */
    {
      if ((e = (*nets[i]->n_init)(i)) != 0) /* call init routine */
      {
        ...
        TCIP_INICHE_TRACE_02(0, LSA_TRACE_LEVEL_WARN,
                             "init error %d on net[%d]", e, i);
#else
       ...
       */
      if (nets[i]->n_mib->ifType == ETHERNET) nets[i]->n_flags |= NF_BCAST;
...
void fixup_subnet_mask(int netnum) /* which of the nets[] to do. */
{
  u_long smask;
...
void netclose() {
  ...
  if (NDEBUG & INFOMSG)
#if 1 /* INICHE_TCIP_ITGR added LSA trace */
    TCIP_INICHE_TRACE_00(0, LSA_TRACE_LEVEL_CHAT, "netclose() called");
#else
  ...
  for (ifp = (NET)netlist.q_head; ifp; ifp = ifp->n_next) {
    if (ifp->n_close) {
#if 1 /* INICHE_TCIP_ITGR added LSA trace */
      TCIP_INICHE_TRACE_00(
          0, LSA_TRACE_LEVEL_WARN,
          "netclose: closing iface XX"); /*AP00990756, AP01366119 */
#else
...
int add_share_route(PACKET pkt) {
   ...
   */
  if ((ifp->n_ipaddr & ifp->snmask) == (pip->ip_src & ifp->snmask)) {
    nexthop = pip->ip_src;
  } else if (rt)
...
#if 1 /* INICHE_TCIP_ITGR AP00901717: ad drop all pakets from a sending \
         mulicast MAC */
    if ((*(pkt->nb_buff + ETHHDR_BIAS + 6) & 0x01) ==
        1) /* if sender has Multicast MAC drop this packet */
    {
      LOCK_NET_RESOURCE(FREEQ_RESID);
      pk_free(pkt); /* dump packet from downed interface */
      UNLOCK_NET_RESOURCE(FREEQ_RESID);
        ...
        case ETHERNET:
...
#ifdef TCIP_CFG_VLAN /*INICHE_TCIP_ITGR enable support of VLAN tagged ethernet \
                        frames (IEEE 802.1 Q)*/
          ...
          if ((unshort)ET_TYPE_GET(eth) == 0x8100) {
            ...
            pkt->nb_prot = pkt->nb_buff +
                           ETHHDR_SIZE /*+ ETHHDR_BIAS see 1220027*/ +
                           4; /*AP00922404 */
...
u_long c_older(u_long ct1, u_long ct2) {
...
int ip2mac(PACKET pkt, /* the packet itself, all set but for dest MAC address */
...
int myloss(void) {
  if (myrand() < (myrandNum * 255) / myrandDenom) return 1;
