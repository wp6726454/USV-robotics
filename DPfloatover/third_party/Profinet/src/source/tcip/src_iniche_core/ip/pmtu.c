...
int pmtucache_init(void) {
  ...
  pmtucache = (struct pmtucacheent *)PMTU_ALLOC(sizeof(struct pmtucacheent) *
                                                pmtu_maxcache);

  if (pmtucache == NULL) return ENP_NOMEM;
 ...
 *          host IP address.
 */
static unsigned pmtu_firstguess(ip_addr dest) {
  ip_addr firsthop;
  NET out_net;
 ...
 */
static unsigned pmtu_nextguess(unsigned estimate) {
  /* If the previous estimate is equal to or below the lower
   * limit, keep it.
   */
  if (estimate <= pmtu_llimit) return estimate;
  ...
  return LSA_MAX(estimate >> 1, pmtu_llimit);
}
 ...
 *         a path MTU estimate.
 */
int pmtucache_set(ip_addr dest, unsigned estimate) {
  int i;
  ...
  for (i = 0; i < pmtu_maxcache; i++) {
    ...
    if ((lru == NULL) || (pmtucache[i].lastused < lru->lastused) ||
        (pmtucache[i].dest == 0)) {
      lru = &pmtucache[i];
    }
  }
...
unsigned pmtucache_get(ip_addr dest) {
  ...
  if ((pmtucache_mru != NULL) && (pmtucache_mru->dest == dest)) {
  get:
  ...
  for (i = 0; i < pmtu_maxcache; i++) {
    if (pmtucache[i].dest != 0) {
      ns_printf(pio, "%u.%u.%u.%u   %d   %lu   %lu   %s%s\n",
                PUSH_IPADDR(pmtucache[i].dest), pmtucache[i].pmtu,
                pmtucache[i].lastused, pmtucache[i].lastchanged,
