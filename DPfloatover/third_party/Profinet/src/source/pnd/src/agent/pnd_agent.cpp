...
/* prototypes                                                                */
/*---------------------------------------------------------------------------*/
static PNIO_UINT32 pnd_agent_iob_core_startup( PND_PARAM_STARTUP_PTR_TYPE pnd_startup_param );
...
static PNIO_UINT32 pnd_agent_pnio_user_core_startup( PND_PARAM_STARTUP_PTR_TYPE pnd_startup_param );
    ...
    PNIO_SYSTEM_DESCR           * pSysDescr)
{
    ...
    if ( paramObj != 0 ) 
    {
        ...
        if ( result == PNIO_OK || result == PNIO_WARN_NO_DECENTRALIOSYSTEM ) // param config correct ?
        {
            EPS_OPEN_TYPE uOpen;
            PNIO_UINT16 hdNr = 0;
...
PNIO_UINT32 PNIO_CODE_ATTR SERV_CP_init(PNIO_DEBUG_SETTINGS_PTR_TYPE  DebugSetting)
{
    ...
    PND_STATIC_ASSERT (sizeof(PNIO_VOID_PTR_TYPE) == sizeof(PNIO_VOID_PTR_TYPE), "must have same size");
...
PNIO_UINT32 PNIO_CODE_ATTR SERV_CP_undo_init ()
{
    ...
    if(pnd_instance.serv_cp_init_done == PNIO_FALSE)
        return PNIO_ERR_SEQUENCE;
...
PNIO_VOID pnd_wpcapdrv_find_boards(EPS_WPCAPDRV_STORE_PTR_TYPE pPndWpcapDrv)
{
    ...
    if(result != -1)
    {
        ...
        {
            pnd_memcpy(dev_name, dev->name + pnd_strlen(PCAP_SRC_IF_STRING), pnd_strlen(dev->name) - pnd_strlen(PCAP_SRC_IF_STRING));
            ...
            if( PacketRequest(lpAdapter, PNIO_FALSE, &buf.pkt) && buf.pkt.Length == 6 )
            {
                ...
                pnd_memcpy(pPndWpcapDrv->foundDevices[pPndWpcapDrv->uCntFoundDevices].uMac, buf.pkt.Data, sizeof(pPndWpcapDrv->foundDevices[pPndWpcapDrv->uCntFoundDevices].uMac));
...
PNIO_UINT32 pnd_get_mac_wpcapdrv( PNIO_CP_ID_PTR_TYPE cp_list, PNIO_UINT8 *NumberOfCps)
{
...
PNIO_UINT32 pnd_get_pci_pndevdrv( PNIO_CP_ID_PTR_TYPE cp_list, PNIO_UINT8 *NumberOfCps )
{
    PNIO_CP_ID_PTR_TYPE pCpList = cp_list;
