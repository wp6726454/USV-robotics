...
typedef enum { PNIO_DA_FALSE = 0, PNIO_DA_TRUE = 1 } PNIO_DEV_ACT_TYPE;
...
typedef struct {
  PNIO_ALARM_TYPE
      AlarmType; /* original alarm-identifier from alarm-data-block */
  PNIO_APRIO_TYPE
      AlarmPriority; /* alarm priority for net, high or low -> see cm defines */
  ...
  PNIO_UINT16
      CpuAlarmPriority; /* only relevant with process alarm, ob number */
  PNIO_UINT32 PnioCompatModtype; /* only relevant for plug/pull alarms: real ==
                                    expected => 0x8101   */

  PNIO_CTRL_RALRM_TINFO AlarmTinfo; /* see struct definition*/

  PNIO_UINT8
      DiagDs[4]; /* only relevant with diagnostic alarm, all 0x0 otherwise */
  PNIO_UINT8
      PrAlarmInfo[4]; /* only relevant with process alarm, all 0x0 otherwise */

  PNIO_CTRL_ALARM_INFO AlarmAinfo; /* see struct definition, only relevant if
                                      not device failure/return alarm  */
...
typedef struct {
  ...
  PNIO_COM_TYPE
      ComType; /* communication type: controller-device, direct data exchange */
...
#define PNIO_CTRL_DEV_DIAG_OFFLINE                        \
  0x00000001 /* device isn't switched on (or)             \
                       isn't attainable over the net (or) \
                       has wrong device name */
#define PNIO_CTRL_DEV_DIAG_ADDR_INFO_OK \
  0x00000010 /* name resolution succeded (device name -> IP) */
#define PNIO_CTRL_DEV_DIAG_AR_CONNECTING 0x00000100 /* intermediate state */
#define PNIO_CTRL_DEV_DIAG_AR_IN_DATA                     \
  0x00000200 /* SRT: connection was built up (or)         \
                RT: intermediate state (or)               \
                RT: device is attached by wrong port (or) \
                RT: network configuration is not equal to STEP7 project */
#define PNIO_CTRL_DEV_DIAG_AR_OFFLINE                       \
  0x00000400 /* SRT: see PNIO_CTRL_DIAG_DEVICE::Reason (or) \
                RT: intermediate state */
#define PNIO_CTRL_DEV_DIAG_AR_RTC3_FOLLOWS  \
  0x00001000 /* RT: intermediate state (or) \
...
typedef struct {
  ...
  PNIO_UINT16 ErrorCause; /* big-endian(!) */
  PNIO_UINT8 ReasonCode;
  PNIO_UINT8 AdditionalInfo[10]; /* big-endian(!) */
...
typedef struct {
  ...
  PNIO_UINT8
      TypeOfStation[256]; /* ctrl 'device type' (Geraetetyp ) TypeOfStation*/
  PNIO_UINT32 ip_addr;
  PNIO_UINT32 ip_mask;
...
typedef struct {
  ...
  PNIO_BOOL Remanent;
} ATTR_PACKED PNIO_IPv4;

typedef struct {
  PNIO_UINT8 Nos[256];
  PNIO_UINT16 Length;
  PNIO_BOOL Remanent;
} ATTR_PACKED PNIO_NOS;
...
} ATTR_PACKED PNIO_IOS_RECONFIG_TYPE, *PNIO_IOS_RECONFIG_PTR_TYPE;
...
typedef enum {
  ...
  PNIO_CBE_CP_STOP_REQ = 6
    ...
    !defined                 \
        PNIO_PNDRIVER /* not supported for PNIO SOFTNET and PNIO PNDRIVER*/
  ,
...
typedef struct { PNIO_MODE_TYPE Mode; } ATTR_PACKED PNIO_CBE_PRM_MODE_IND;
...
typedef struct {
  ...
  PNIO_UINT32 Result;
    ...
    !defined                 \
        PNIO_PNDRIVER /* not supported for PNIO SOFTNET and PNIO PNDRIVER*/
typedef struct { PNIO_UINT32 Frequency; } ATTR_PACKED PNIO_CBE_PRM_LED_FLASH;
#endif /* not supported for PNIO SOFTNET and PNIO PNDRIVER */
...
typedef struct {
  ...
  PNIO_UINT8*
      pDiagDataBuffer; /* cast to PNIO_CTRL_DIAG_CONFIG_SUBMODULE[...] or
...
typedef struct {
  ...
  PNIO_UINT32 Options;
  PNIO_UINT32 Err;
} ATTR_PACKED PNIO_CBE_PRM_SET_ADDR_CONF;

typedef struct {
  PNIO_UINT8* RemaXMLBuffer;
  PNIO_UINT32 RemaXMLBufferLength;
  PNIO_UINT32 Err;
} ATTR_PACKED PNIO_CBE_PRM_REMA_READ_IND;

typedef struct {
  PNIO_ADDR* pAddr;
  PNIO_DEV_ACT_TYPE Mode;
  PNIO_UINT32 Status;
} ATTR_PACKED PNIO_CBE_PRM_IOSYS_CONF;
...
#endif /* not supported for PNIO SOFTNET and PNDRIVER */
...
PNIO_controller_open(PNIO_UINT32 CpIndex,         /* in */
...
PNIO_UINT32 PNIO_CODE_ATTR
PNIO_interface_open(PNIO_UINT32 CpIndex, PNIO_CBF cbf_RecReadConf, /* in */
                    PNIO_CBF cbf_AlarmInd,                         /* in */
                    PNIO_UINT32* Handle);                          /* in */

PNIO_UINT32 PNIO_CODE_ATTR
PNIO_device_activate(PNIO_UINT32 Handle,            /* in */
                     PNIO_ADDR* pAddr,              /* in */
                     PNIO_DEV_ACT_TYPE DeviceMode); /* in */

PNIO_UINT32 PNIO_CODE_ATTR PNIO_set_mode(PNIO_UINT32 Handle,   /* in */
                                         PNIO_MODE_TYPE Mode); /* in */

PNIO_UINT32 PNIO_CODE_ATTR PNIO_rec_read_req(PNIO_UINT32 Handle,      /* in */
                                             PNIO_ADDR* pAddr,        /* in */
                                             PNIO_UINT32 RecordIndex, /* in */
                                             PNIO_REF ReqRef,         /* in */
                                             PNIO_UINT32 Length);     /* in */

PNIO_UINT32 PNIO_CODE_ATTR PNIO_rec_write_req(PNIO_UINT32 Handle,      /* in */
                                              PNIO_ADDR* pAddr,        /* in */
                                              PNIO_UINT32 RecordIndex, /* in */
                                              PNIO_REF ReqRef,         /* in */
                                              PNIO_UINT32 Length,      /* in */
...
PNIO_UINT32 PNIO_CODE_ATTR
PNIO_ctrl_diag_req(PNIO_UINT32 Handle,        /* in */
                   PNIO_CTRL_DIAG* pDiagReq); /* in */

#endif /* not supported for PNIO SOFTNET */

PNIO_UINT32 PNIO_CODE_ATTR
PNIO_iosystem_reconfig(PNIO_UINT32 Handle,                  /* in */
...
PNIO_UINT32 PNIO_CODE_ATTR PNIO_interface_close(PNIO_UINT32 Handle);  /* in */
#endif

PNIO_UINT32 PNIO_CODE_ATTR PNIO_data_read(PNIO_UINT32 Handle,      /* in */
                                          PNIO_ADDR* pAddr,        /* in */
    ...
    !defined                 \
        PNIO_PNDRIVER /* not supported for PNIO SOFTNET and PNIO PNDRIVER*/
PNIO_UINT32 PNIO_CODE_ATTR
PNIO_output_data_read(PNIO_UINT32 Handle,      /* in */
                      PNIO_ADDR* pAddr,        /* in */
                      PNIO_UINT32 BufLen,      /* in */
                      PNIO_UINT32* pDataLen,   /* out */
                      PNIO_UINT8* pBuffer,     /* out */
...
PNIO_UINT32 PNIO_CODE_ATTR
PNIO_data_read_cache_refresh(PNIO_UINT32 Handle); /* in */

PNIO_UINT32 PNIO_CODE_ATTR
PNIO_data_read_cache(PNIO_UINT32 Handle,      /* in */
                     PNIO_ADDR* pAddr,        /* in */
                     PNIO_UINT32 BufLen,      /* in */
                     PNIO_UINT32* pDataLen,   /* out */
                     PNIO_UINT8* pBuffer,     /* out */
...
PNIO_UINT32 PNIO_CODE_ATTR PNIO_data_write(PNIO_UINT32 Handle,      /* in */
                                           PNIO_ADDR* pAddr,        /* in */
                                           PNIO_UINT32 BufLen,      /* in */
                                           PNIO_UINT8* pBuffer,     /* in */
                                           PNIO_IOXS IOlocState,    /* in */
    ...
    !defined                 \
        PNIO_PNDRIVER /* not supported for PNIO SOFTNET and PNIO PNDRIVER*/
PNIO_UINT32 PNIO_CODE_ATTR
PNIO_data_write_cache_flush(PNIO_UINT32 Handle); /* in */

PNIO_UINT32 PNIO_CODE_ATTR
PNIO_data_write_cache(PNIO_UINT32 Handle,      /* in */
                      PNIO_ADDR* pAddr,        /* in */
                      PNIO_UINT32 BufLen,      /* in */
                      PNIO_UINT8* pBuffer,     /* in */
                      PNIO_IOXS IOlocState,    /* in */
...
PNIO_UINT32 PNIO_CODE_ATTR PNIO_data_cache_exchange(PNIO_UINT32 Handle);
#endif /* not supported for PNIO SOFTNET and PNIO PNDRIVER */

PNIO_UINT32 PNIO_CODE_ATTR PNIO_register_cbf(PNIO_UINT32 Handle,    /* in */
                                             PNIO_CBE_TYPE CbeType, /* in */
                                             PNIO_CBF Cbf);         /* in */

PNIO_UINT32 PNIO_CODE_ATTR
PNIO_interface_register_cbf(PNIO_UINT32 Handle,    /* in */
                            PNIO_CBE_TYPE CbeType, /* in */
                            PNIO_CBF Cbf);         /* in */

PNIO_UINT32 PNIO_CODE_ATTR
PNIO_interface_rec_read_req(PNIO_UINT32 Handle,      /* in */
                            PNIO_ADDR* pAddr,        /* in */
                            PNIO_UINT32 RecordIndex, /* in */
                            PNIO_REF ReqRef,         /* in */
                            PNIO_UINT32 Length);     /* in */
...
PNIO_UINT32 PNIO_CODE_ATTR
PNIO_interface_set_ip_and_nos(PNIO_UINT32 Handle,             /* in */
...
typedef struct {
  ...
  PNIO_UINT8
      power_mode; /* 0x00=power on, 0x01=power off during power save mode */
...
typedef struct {
  ...
  PNIO_PE_CFG_SLOT
      slot[PNIO_PE_CFG_DEV_NUM_SLOTS_MAX]; /* array of slot config blocks */
} ATTR_PACKED PNIO_PE_PRM_CONFIG_DEVICE_REQ;

/* Alignment test: sizeof PNIO_PE_PRM_CONFIG_DEVICE_REQ must be == 18
*/
typedef char _size_tst_config_dev_rec_[(
    (sizeof(PNIO_PE_PRM_CONFIG_DEVICE_REQ) == 18) ? 1 : -1)];
...
/* PE_SAP_DR_INDEX : subslot specific PROFIenergy Service Access Point 'Data
 * Record Index'
 * PE_DR_INDEX_MASK: 0x80A1..0x80AF: subslot specific record data reserved for
 * further use
 */
#define PE_SAP_DR_INDEX 0x80A0
#define PE_DR_INDEX_MASK 0x80A0

/* PNIO_PE_SERVICE_REQ_LIFETIME_DEFAULT: Time in sec. to wait for Service
 * Response
*/
...
/* PE command Query_Measurement:  Q_MSRMT_GET_LIST     Service_Modifier= 0x01 ==
...
/* PE command Query_Measurement:  Q_MSRMT_GET_VAL      Service_Modifier= 0x02 ==
 ...
 */
PNIO_UINT32 PNIO_CODE_ATTR PNIO_pe_cmd_req(PNIO_UINT32 Handle,        /* in */
                                           PNIO_ADDR* pAddr,          /* in */
                                           PNIO_REF ReqRef,           /* in */
...
PNIO_UINT32 PNIO_CODE_ATTR PNIO_register_pe_cbf(PNIO_UINT32 Handle, /* in */
                                                PNIO_PE_CBF Cbf     /* in */
                                                );
