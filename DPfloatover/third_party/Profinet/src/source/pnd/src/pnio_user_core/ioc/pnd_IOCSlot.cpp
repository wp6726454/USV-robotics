...
string CIOCSlot::GetKey( LSA_UINT32 api, LSA_UINT16 slot, LSA_UINT16 subSlot )
{
    CValueHelper valObj;
...
/*----------------------------------------------------------------------------*/
LSA_UINT32 CIOCSlot::GetApiNr()
{
    return( m_api );
}

/*----------------------------------------------------------------------------*/
LSA_UINT16 CIOCSlot::GetSlotNr()
{
    return( m_slotNr );
}
...
/*----------------------------------------------------------------------------*/
LSA_UINT16 CIOCSlot::GetSlotLogAddr()
{
    return ( m_slotLaddr );
}

/*----------------------------------------------------------------------------*/
LSA_UINT16 CIOCSlot::GetSubslotLogAddr()
{
    return ( m_subslotLaddr );
}

/*----------------------------------------------------------------------------*/
LSA_UINT32 CIOCSlot::GetSlotIdent()
{
    return ( m_slotIdent );
}

/*----------------------------------------------------------------------------*/
LSA_UINT32 CIOCSlot::GetSubslotIdent()
{
    return ( m_subslotIdent );
}

/*----------------------------------------------------------------------------*/
PND_IO_ADDR_TYPE CIOCSlot::GetAddressForInput()
{
    return ( m_input );
}

/*----------------------------------------------------------------------------*/
PND_IO_ADDR_TYPE CIOCSlot::GetAddressForOutput()
{
    return ( m_output );
}
...
LSA_VOID CIOCSlot::ReadRecordAR( PND_RQB_PTR_TYPE p_rb )
{
    ...
    PND_IOC_TRACE_09( m_pndHandle.pnio_if_nr, LSA_TRACE_LEVEL_NOTE,
        "CIOCSlot::ReadRecordAR(): IOC READ record, dev(%u) ar(%u) sesKey(%u) api(%#x) slot(%#x) sub(%#x) recIdx(%#x) dataSize(%u) RQB(0x%08x)", 
        devNr, 
        arNr, 
        sesKey,
        m_api, 
    ...
    eps_user_request_lower( pRQB, this, CIOCSlot::ReadRecordARDone );
}

/*----------------------------------------------------------------------------*/
LSA_VOID CIOCSlot::ReadRecordImplicit( PND_RQB_PTR_TYPE p_rb )
{
    LSA_HANDLE_TYPE handle = GetPnioHandle();
    ...
    PND_IOC_TRACE_06( m_pndHandle.pnio_if_nr, LSA_TRACE_LEVEL_NOTE,
        "CIOCSlot::ReadRecordImplicit(): IOC READ record implicit, api(%#x) slot(%#x) sub(%#x) recIdx(%#x) dataSize(%u) RQB(0x%08x)", 
        m_api, 
        m_slotNr, 
        m_subslotNr, 
        pRead->RecordIndex, 
    ...
    eps_user_request_lower( pRQB, this, CIOCSlot::ReadRecordImplicitDone );
}

/*----------------------------------------------------------------------------*/
LSA_VOID CIOCSlot::WriteRecord(PND_RQB_PTR_TYPE p_rb)
{
    LSA_HANDLE_TYPE  handle = GetPnioHandle();
    ...
    PND_IOC_TRACE_09( m_pndHandle.pnio_if_nr, LSA_TRACE_LEVEL_NOTE,
        "CIOCSlot::WriteRecord(): IOC WRITE record, dev(%u) ar(%u) sesKey(%u) api(%#x) slot(%#x) sub(%#x) recIdx(%#x) dataSize(%u) RQB(0x%08x)", 
        devNr, 
        arNr, 
        sesKey,
        m_api, 
    ...
    eps_user_request_lower( pRQB, this, CIOCSlot::WriteRecordDone );
}
...
LSA_VOID CIOCSlot::ReadRecordARDone( LSA_VOID* p_inst, LSA_VOID* p_rb )
{
    ...
    PND_IOC_TRACE_06( pSub->m_pndHandle.pnio_if_nr, LSA_TRACE_LEVEL_NOTE,
        "CIOCSlot::ReadRecordARDone(): IOC Record Read done, rsp(%#x) pnio_stat(%#x) dev(%u) ar(%u) sesKey(%u) RQB(0x%08x)", 
        rsp,
        pnioErr,
    ...
    PND_IOC_TRACE_06( pSub->m_pndHandle.pnio_if_nr, LSA_TRACE_LEVEL_NOTE,
        "CIOCSlot::ReadRecordARDone():    api(%#x) slot(%#x) sub(%#x) recIdx(%#x) dataSize(%u/%x)", 
...
/*----------------------------------------------------------------------------*/
LSA_VOID CIOCSlot::ReadRecordImplicitDone( LSA_VOID* p_inst, LSA_VOID* p_rb )
{
    CIOCSlot *pSub          = static_cast<CIOCSlot*>(p_inst);
    CM_UPPER_RQB_PTR_TYPE pRQB = static_cast<CM_UPPER_RQB_PTR_TYPE>(p_rb);
    ...
    PND_IOC_TRACE_04( pSub->m_pndHandle.pnio_if_nr, LSA_TRACE_LEVEL_NOTE,
        "CIOCSlot::ReadRecordImplicitDone(): IOC Record Read Implicit done, rsp(%#x) lowerRsp(%#x) pnioErr(%#x) RQB(0x%08x)", 
        rsp,
    ...
    PND_IOC_TRACE_06( pSub->m_pndHandle.pnio_if_nr, LSA_TRACE_LEVEL_NOTE,
        "CIOCSlot::ReadRecordImplicitDone():    api(%#x) slot(%#x) sub(%#x) recIdx(%#x) dataSize(%u/%#x)", 
...
/*----------------------------------------------------------------------------*/
LSA_VOID CIOCSlot::WriteRecordDone( LSA_VOID* p_inst, LSA_VOID* p_rb )
{
    CIOCSlot *pSub          = static_cast<CIOCSlot*>(p_inst);
    CM_UPPER_RQB_PTR_TYPE pRQB = static_cast<CM_UPPER_RQB_PTR_TYPE>(p_rb);
    ...
    PND_IOC_TRACE_06( pSub->m_pndHandle.pnio_if_nr, LSA_TRACE_LEVEL_NOTE,
        "CIOCSlot::WriteRecordDone(): IOC Record Write done, rsp(%#x) pnioErr(%#x) dev(%u) ar(%u) sesKey(%u) RQB(0x%08x)", 
        rsp,
        pnioErr,
    ...
    PND_IOC_TRACE_05( pSub->m_pndHandle.pnio_if_nr, LSA_TRACE_LEVEL_NOTE,
        "CIOCSlot::WriteRecordDone():    api(%#x) slot(%#x) sub(%#x) recIdx(%#x) dataSize(%u)", 
