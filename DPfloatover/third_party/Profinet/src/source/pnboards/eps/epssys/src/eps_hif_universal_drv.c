...
LSA_VOID   eps_hif_universal_drv_uninstall       (LSA_VOID);
LSA_UINT16 eps_hif_universal_drv_open            (struct eps_hif_drv_openpar_tag* pInPars, EPS_HIF_DRV_HW_PTR_TYPE* ppHwInstOut, LSA_UINT16 hd_id, LSA_BOOL bUpper);
LSA_UINT16 eps_hif_universal_drv_close           (EPS_HIF_DRV_HW_PTR_TYPE pHwInstIn);
LSA_UINT16 eps_hif_universal_drv_enable_isr      (EPS_HIF_DRV_HW_PTR_TYPE pHwInstIn, LSA_UINT16 hHif);
LSA_UINT16 eps_hif_universal_drv_disable_isr     (EPS_HIF_DRV_HW_PTR_TYPE pHwInstIn);
LSA_UINT16 eps_hif_universal_drv_send_hif_ir     (EPS_HIF_DRV_HW_PTR_TYPE pHwInstIn);
LSA_UINT16 eps_hif_universal_drv_ack_hif_ir      (EPS_HIF_DRV_HW_PTR_TYPE pHwInstIn);
LSA_UINT16 eps_hif_universal_drv_reenable_hif_ir (EPS_HIF_DRV_HW_PTR_TYPE pHwInstIn);
LSA_VOID   eps_hif_universal_drv_dmacpy          (EPS_HIF_DRV_HW_PTR_TYPE pShmInstIn, LSA_VOID** ppData, LSA_UINT32 uLength);
LSA_VOID   eps_hif_universal_drv_mem_pool_create (EPS_HIF_DRV_HW_PTR_TYPE pShmInstIn, LSA_VOID* pBase, LSA_UINT32 uLength);
LSA_VOID   eps_hif_universal_drv_mem_pool_free   (EPS_HIF_DRV_HW_PTR_TYPE pShmInstIn, LSA_UINT16 *pRetVal);
LSA_VOID   eps_hif_universal_drv_alloc           (EPS_HIF_DRV_HW_PTR_TYPE pShmInstIn, LSA_VOID_PTR_TYPE *ppMem, LSA_UINT32 uLength);
LSA_VOID   eps_hif_universal_drv_free            (EPS_HIF_DRV_HW_PTR_TYPE pShmInstIn, LSA_UINT16 *pRetVal, LSA_VOID_PTR_TYPE pMem);
LSA_VOID   eps_hif_universal_drv_get_params      (EPS_HIF_DRV_HW_PTR_TYPE pShmInstIn, LSA_VOID* pPar);
...
typedef struct
{
	...
	LSA_INT    hHifShmPool;         ///< handle to HIF pool
	...
	LSA_UINT32 uCntAllocs;          ///< statistics
	LSA_UINT32 uCntFrees;           ///< statistics
...
EPS_HIF_UNIVERSAL_DRV_TYPE g_EpsHifUniversalDrv;
EPS_HIF_UNIVERSAL_DRV_PTR_TYPE g_pEpsHifUniversalDrv = LSA_NULL;
...
{
	eps_free_critical_section(g_pEpsHifUniversalDrv->hEnterExit);
}
...
{
	eps_alloc_critical_section(&g_pEpsHifUniversalDrv->hEnterExit, LSA_FALSE);
}
 ...
 */
static EPS_HIF_UNIVERSAL_DRV_INST_PTR_TYPE eps_hif_universal_drv_get_inst (EPS_HIF_DRV_HW_PTR_TYPE pHwInstIn)
{
...
LSA_VOID eps_hif_universal_drv_install (LSA_VOID)
{
	...
	sHifDrvIf.MemPoolCreate = eps_hif_universal_drv_mem_pool_create;
	sHifDrvIf.MemPoolFree   = eps_hif_universal_drv_mem_pool_free;
	sHifDrvIf.MemAlloc      = eps_hif_universal_drv_alloc;
	sHifDrvIf.MemFree       = eps_hif_universal_drv_free;
 ...
 */
LSA_UINT16 eps_hif_universal_drv_open (EPS_HIF_DRV_OPENPAR_PTR_TYPE pInPars, EPS_HIF_DRV_HW_PTR_PTR_TYPE ppHwInstOut, LSA_UINT16 hd_id, LSA_BOOL bUpper)
{
	...
	EPS_SYSTEM_TRACE_02( 0, LSA_TRACE_LEVEL_NOTE_HIGH,"eps_hif_universal_drv_open - Using EPS HIF Short Mode Drv. hd_id=%d, isUpper=%d", hd_id, bUpper);
...
LSA_UINT16 eps_hif_universal_drv_close (EPS_HIF_DRV_HW_PTR_TYPE pHwInstIn)
{
    EPS_HIF_UNIVERSAL_DRV_INST_PTR_TYPE pInst = eps_hif_universal_drv_get_inst(pHwInstIn);
 ...
 * @return LSA_VOID
 */
LSA_VOID eps_hif_universal_drv_dmacpy (EPS_HIF_DRV_HW_PTR_TYPE pHwInstIn, LSA_VOID** ppData, LSA_UINT32 uLength)
{
    EPS_HIF_UNIVERSAL_DRV_INST_PTR_TYPE pInst = eps_hif_universal_drv_get_inst(pHwInstIn);
...
/**
 * Creates a pool for HIF.
 * 
 * @see eps_hif_drv_if_mem_pool_create       - calls this function
 * 
 * @param [in] pHwInstIn                     - handle to instance
 * @param [out] pBase                        - returned pointer 
 * @param [in] uLength                       - requested length of the memory
 * @return LSA_VOID
 */
LSA_VOID eps_hif_universal_drv_mem_pool_create (EPS_HIF_DRV_HW_PTR_TYPE pHwInstIn, LSA_VOID* pBase, LSA_UINT32 uLength)
{
    EPS_HIF_UNIVERSAL_DRV_INST_PTR_TYPE pInst = eps_hif_universal_drv_get_inst(pHwInstIn);
	...
	pInst->hHifShmPool = eps_cp_mem_create_hif_pool(pInst->sHw.hd_id, (LSA_UINT8*)pBase, uLength);

	eps_hif_universal_drv_exit();
}

/**
 * Releases a pool for HIF.
 * 
 * @see eps_hif_drv_if_mem_pool_free    - calls this function
 * 
 * @param [in] pHwInstIn                - handle to instance
 * @param [out] pRetVal                 - returns #LSA_RET_OK as value
 * @return LSA_VOID
 */
LSA_VOID eps_hif_universal_drv_mem_pool_free (EPS_HIF_DRV_HW_PTR_TYPE pHwInstIn, LSA_UINT16 *pRetVal)
{
    EPS_HIF_UNIVERSAL_DRV_INST_PTR_TYPE pInst = eps_hif_universal_drv_get_inst(pHwInstIn);
	...
	eps_cp_mem_delete_hif_pool(pInst->sHw.hd_id, pInst->hHifShmPool);
...
/**
 * This function allocates a memory for HIF.
 * 
 * @see eps_hif_drv_if_alloc_shm  - calls this function
 * 
 * @param [in] pHwInstIn               - handle to instance
 * @param [out] ppMem                  - returned pointer to mem pool
 * @param [in] uLength                 - length of the pool
 * @return LSA_VOID
 */
LSA_VOID eps_hif_universal_drv_alloc (EPS_HIF_DRV_HW_PTR_TYPE pHwInstIn, LSA_VOID_PTR_TYPE *ppMem, LSA_UINT32 uLength)
{
    EPS_HIF_UNIVERSAL_DRV_INST_PTR_TYPE pInst = eps_hif_universal_drv_get_inst(pHwInstIn);
	...
	*ppMem = (LSA_VOID_PTR_TYPE)eps_cp_mem_hif_alloc(uLength, pInst->hHifShmPool, pHwInstIn->bUpper, LSA_COMP_ID_HIF);
}

/**
 * This function frees a memory for HIF.
 * 
 * @see   eps_hif_drv_if_alloc_shm  - calls this function
 * 
 * @param [in] pHwInstIn            - handle to instance
 * @param [out] pRetVal             - returns #LSA_RET_OK as value
 * @param [in] pMem                 - memory that shall be released.
 * @return LSA_VOID
 */
LSA_VOID eps_hif_universal_drv_free (EPS_HIF_DRV_HW_PTR_TYPE pHwInstIn, LSA_UINT16 *pRetVal, LSA_VOID_PTR_TYPE pMem)
{
    EPS_HIF_UNIVERSAL_DRV_INST_PTR_TYPE pInst = eps_hif_universal_drv_get_inst(pHwInstIn);
	...
	eps_cp_mem_hif_free((LSA_UINT8*)pMem, pInst->hHifShmPool, pHwInstIn->bUpper, LSA_COMP_ID_HIF);
 ...
 * @return LSA_VOID
 */
LSA_VOID eps_hif_universal_drv_get_params (EPS_HIF_DRV_HW_PTR_TYPE pHwInstIn, LSA_VOID* pPar)
{
