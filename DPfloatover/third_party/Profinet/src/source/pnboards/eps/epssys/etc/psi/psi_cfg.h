...
#define PSI_ASSERT(cond) \
	if (! (cond)) { PSI_FATAL (0); } else psi_semicolon_missing = 1
...
#define ACP_CFG_MAX_CHANNELS           (4 * PSI_CFG_MAX_PNIO_IF_CNT)// System , UserIOC, UserIOD, UserIOM per NIC
...
#define CLRPC_CFG_MAX_CHANNELS  (3 * PSI_CFG_MAX_PNIO_IF_CNT) // per NIC: 1(PATH_TYPE_EPM) + 2(PATH_TYPE_USER, CM and Easy-Supervisor)
#define CLRPC_CFG_MAX_CLIENTS   ((PSI_MAX_CL_DEVICES + PSI_MAX_SV_DEVICES*PSI_MAX_SV_AR)*PSI_CFG_MAX_PNIO_IF_CNT)
#define CLRPC_CFG_MAX_SERVERS   (1/*EPM*/ + (1/*CM_CL*/ + 1*PSI_MAX_SV_DEVICES/*CM-SV*/) * PSI_CFG_MAX_PNIO_IF_CNT) /* 0 = no server-part */
#define CLRPC_CFG_MAX_SCALLS    ((PSI_MAX_CL_DEVICES + PSI_MAX_SV_DEVICES*(PSI_MAX_SV_AR + PSI_MAX_SV_READ_IMPL))*PSI_CFG_MAX_PNIO_IF_CNT)
#define CLRPC_CFG_MAX_READ_IMPLICIT_PER_OBJ_UUID ( PSI_MAX_SV_READ_IMPL * PSI_CFG_MAX_PNIO_IF_CNT )

/*---------------------------------------------------------------------------*/
/* Configure CM                                                              */
/*---------------------------------------------------------------------------*/

#define PSI_MAX_CONNECTS_PER_SECOND min(50, PSI_MAX_CL_DEVICES) /* Count of devices CM can handle in parallel */ /*(LaM)Changed*/
...
// IOM
#define CM_CFG_MAX_MULTICAST_CHANNELS (PSI_CFG_USE_IOM * PSI_CFG_MAX_PNIO_IF_CNT)
...
#define EDD_CFG_MAX_PORT_CNT_ALL_INTERFACE (PSI_CFG_MAX_PORT_CNT*PSI_CFG_MAX_PNIO_IF_CNT)
...
#define EDDI_CFG_SII_ADD_PRIO1_AUX      // active at all product variants - 2015-02-26

#if (EPS_PLF == EPS_PLF_WINDOWS_X86)    // Light -> Polling Mode - Polling-Cycle ~1ms
#define EDDI_CFG_SII_POLLING_MODE
#elif (EPS_PLF == EPS_PLF_ADONIS_X86)   // Advanced -> Polling Mode - Polling-Cycle ~1ms
#define EDDI_CFG_SII_POLLING_MODE
#elif (EPS_PLF == EPS_PLF_LINUX_X86)    // Linux -> Polling Mode - Polling-Cycle ~1ms
#define EDDI_CFG_SII_POLLING_MODE                                         
#elif (EPS_PLF == EPS_PLF_SOC_MIPS)         // BASIC MIPS (Soc) - ExtTimer + Interrupt
#define EDDI_CFG_SII_EXTTIMER_MODE_ON       // this settings are identical to PLC 15xx - 2015-02-26
//#define EDDI_CFG_SII_REMOVE_ISR_LOCKS     // currently not allowed, because eps calls eddi_interrupt in thread context (not in interrupt context)
...
#define HIF_CFG_MAX_HD_INSTANCES     (PSI_CFG_MAX_HD_CNT*2)  // ableiten aus MAX HD *2 fuer Upper/lower
...
#define HIF_CFG_MAX_HD_INSTANCES   (PSI_CFG_MAX_HD_CNT*2)    // anzahl der Hw Devices * 2
#endif
#endif
...
#define HIF_CFG_USE_CPHD_APP_SOCK_CHANNELS 0 // Don't use app specific HD Sock channels
...
//#define HIF_ALLOC_LOCAL_MEM_WITH_NO_MEMSET //TODO still fixes in hif necassary to remove memset of local allocated memory
#define HIF_DEBUG_MEASURE_PERFORMANCE
#else
#define HIF_CFG_ALIGNMENT_SAVE_COPY_FUNCTION // attention: for arm platforms (ertec200p, ertec200, ertec400) this define has to be set! 32 bit copy does not work for misaligned 8/16 bit enums!
#endif
...
#define NARE_CFG_MAX_USER_CHANNELS (3* PSI_CFG_MAX_PNIO_IF_CNT) /* CM, OHA, NareAppChannel */
...
#define OHA_CFG_MAX_CHANNELS  (1 /* global */ + (4 * PSI_CFG_MAX_PNIO_IF_CNT /* IF User */))
...
#define PNTRC_CFG_SYNC_INTERVAL        500 /* 500 ms Time Sync Interval to lower systems */
...
#define SOCK_CFG_MAX_CHANNELS      ( 3 + PSI_CFG_MAX_SOCK_APP_CHANNELS )// global SOCK App Users + CLRPC + SNMPX + OHA /*(LaM)Changed*/
#define SOCK_CFG_MAX_INTERFACE_CNT PSI_CFG_MAX_PNIO_IF_CNT              // same as EDD_CFG_MAX_INTERFACE_CNT
...
#endif
#define PSI_CFG_MAX_SOCKETS         (PSI_MAX_SOCKETS_PCIOX_STATIC + PSI_MAX_SV_DEVICES*PSI_CFG_MAX_PNIO_IF_CNT +  PSI_MAX_SOCKETS_IOC*PSI_CFG_MAX_PNIO_IF_CNT)
...
#define TCIP_CFG_SOMAXCONN              10 /* maximum length of the TCP backlog (1-127, 0 defaults to 5) */
...
#define IOH_CFG_MAX_RUNNING_ISO_JOBS      16  //max number of isochronous jobs per IF

#define IOH_CFG_MAX_PI_BUFFERS_IOC   (PSI_MAX_CL_DEVICES)                         // max number for IOC
#define IOH_CFG_MAX_PI_BUFFERS_IOD   (PSI_MAX_SV_DEVICES*PSI_MAX_SV_IO_ARS)       // max number for IOD

#define IOH_CFG_MAX_PI_BUFFERS LSA_MAX(IOH_CFG_MAX_PI_BUFFERS_IOC, IOH_CFG_MAX_PI_BUFFERS_IOD) // either IOD or IOC
