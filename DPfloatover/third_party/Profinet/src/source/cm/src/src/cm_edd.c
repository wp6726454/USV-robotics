...
cm_edd_channel_init(
	...
	if(
		channel->lower.info.edd.MaxPortCnt < 1 /* at least 1 port */
		|| channel->lower.info.edd.MaxPortCnt > EDD_CFG_MAX_PORT_CNT /* wrong configuration */
	) {
...
cm_edd_get_apdu_status_length(
	...
	if( (channel->lower.info.edd.HardwareType & EDD_HW_TYPE_FEATURE_APDUSTATUS_SEPARATE_MSK) == EDD_HW_TYPE_FEATURE_APDUSTATUS_SEPARATE ) {
...
)
{
	...
	CM_ASSERT (is_not_null(lower_rqb_ptr_ptr));
		...
		/* alloc the RQB */
	CM_EDD_ALLOC_LOWER_RQB (lower_rqb_ptr_ptr, cm_null_user_id, sizeof (CM_EDD_LOWER_RQB_TYPE), edd_channel->sysptr);
	...
	case 0: /* special case, one for all, see open/close/prm */
		length = LSA_MAX(sizeof(EDD_RQB_OPEN_CHANNEL_TYPE), sizeof(EDD_RQB_GET_LINK_STATUS_TYPE));
...
cm_edd_send_clock_range_ok(
	...
	if( send_clock < 1 ) { /* note: SCF < 8 is supported since V5.0i2 (AP01035332) */
	...
	default: /* non-power-of-two */
		...
		if( is_null(is_power_of_two) ) { /* flag: non-power-of-two values are not allowed */

			CM_EDD_TRACE_01(channel->trace_idx, LSA_TRACE_LEVEL_NOTE
				, "send_clock(%u) not a power of two"
				, send_clock
				);
...
cm_edd_prm_indication(
	...
	if( is_null(edd) ) { /* provide */
		...
		for( port_id = 0; port_id <= channel->lower.info.edd.MaxPortCnt; ++port_id ) {
		...
		if( channel->state >= CM_CH_CLOSE ) {
			...
			*/
			...
			cm_edd_request_lower(channel, EDD_SRV_PRM_INDICATION, 0, 0, edd); /* re-provide */
		}
	}
}
...
cm_edd_request_lower(
	...
	CM_EDD_TRACE_06 (channel->trace_idx, LSA_TRACE_LEVEL_NOTE
		...
		, CM_EDD_GET_USER_ID_HANDLE (edd), CM_EDD_GET_USER_ID_DEV (edd), CM_EDD_GET_USER_ID_AR (edd)
		);
...
cm_edd_request_lower_done (
	...
	case EDD_STS_OK_CANCEL:
		CM_EDD_TRACE_06 (channel->trace_idx, LSA_TRACE_LEVEL_NOTE
			, "<<< edd-rqb(0x%08x) edd-svc(0x%x) edd-rsp(%u) t-h(%u) t-dev(%u) t-ar(%u)"
	...
	case CM_PATH_TYPE_EDD:
		...
		case EDD_OPC_REQUEST:
			...
			case EDD_SRV_SENDCLOCK_CHANGE: /* "set-send-clock 0" before closing the edd-channel */
				if( CM_EDD_GET_RESPONSE(edd) != EDD_STS_OK ) {
