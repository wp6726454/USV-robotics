...
bool CNetwork::Connect(char *pServerAddr, unsigned short nPort) {
  ...
  if (setsockopt(mhSocket, IPPROTO_TCP, TCP_NODELAY, &bNoDelay,
                 sizeof(bNoDelay))) {
    SetErrorString();
...
bool CNetwork::Connected() { return mhSocket != INVALID_SOCKET; }
...
int CNetwork::Receive(char *rtDataBuff, int nDataBufSize, bool bHeader,
                      int nTimeout, unsigned int *ipAddr) {
  int nRecved = 0;
  ...
  if (nSelectRes > 0) {
    if (FD_ISSET(mhSocket, &ExceptFDs)) {
      ...
      nRecved = recvfrom(mhUDPSocket, rtDataBuff, nDataBufSize, 0,
      ...
      nRecved = recvfrom(mhUDPBroadcastSocket, rtDataBuff, nDataBufSize, 0,
                         (sockaddr *)&source_addr, &fromlen);
...
bool CNetwork::SendUDPBroadcast(const char *pSendBuf, int nSize, short nPort,
      ...
      while (pNextAd) {
        if (pNextAd->Type == MIB_IF_TYPE_ETHERNET) {
          ...
          if (inet_pton(AF_INET, pNextAd->IpAddressList.IpAddress.String,
                        &nIPaddr) == NULL ||
              inet_pton(AF_INET, pNextAd->IpAddressList.IpMask.String,
                        &nIPmask) == NULL) {
            return false;
          }
          recvAddr.sin_addr.s_addr = nIPaddr | (~nIPmask);
          if (recvAddr.sin_addr.s_addr != (nFilterAddr | (~nIPmask))) {
            if (sendto(mhUDPBroadcastSocket, pSendBuf, nSize, 0,
                       (sockaddr *)&recvAddr, sizeof(recvAddr)) == nSize) {
              bBroadCastSent = true;
            }
          }
        }
...
void CNetwork::SetErrorString() {
  ...
  FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM,
                NULL, mnLastError, 0, reinterpret_cast<LPTSTR>(&tError), 0,
                NULL);
