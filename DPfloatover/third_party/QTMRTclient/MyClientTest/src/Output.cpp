...
void COutput::HandleDataFrame(FILE* logfile, bool bLogMinimum, CRTProtocol* poRTProtocol, CInput::EOperation operation, bool bOutputModeScrolling)
{
    ...
    mfCurrentRecvTime = (1.0 * (nCurrentTime.QuadPart - mnStartTime.QuadPart)) / (1.0 * mnFrequency.QuadPart);
    ...
    if (mfLastScreenUpdateTime == 0.0 || (mfCurrentRecvTime - mfLastScreenUpdateTime) > 0.1)
    {
