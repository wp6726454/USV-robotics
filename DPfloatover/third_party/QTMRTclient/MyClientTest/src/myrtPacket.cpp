...
unsigned __int64 CRTPacket::GetTimeStamp(char *pData, bool bBigEndian) {
  if (GetType(pData, bBigEndian) == PacketData) {
    if (bBigEndian) {
      return ((unsigned __int64)(ntohl((long)*((__int64 *)(pData + 8))))
              << 32) +
             ntohl(*((__int64 *)(pData + 8)) >> 32);
    }
...
double CRTPacket::SetByteOrder(double *pfData) {
  ...
  if (mbBigEndian) {
    nTmp = (((unsigned __int64)(ntohl((long)*((unsigned __int64 *)pfData)))
             << 32) +
...
__int64 CRTPacket::SetByteOrder(__int64 *pnData) {
  if (mbBigEndian) {
    return ((unsigned __int64)(ntohl((long)*pnData)) << 32) +
           ntohl(*pnData >> 32);
  }
  return *pnData;
}  // SetByteOrder
