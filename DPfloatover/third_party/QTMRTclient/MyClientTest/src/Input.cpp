...
char CInput::WaitForKey() { return _getch(); }
...
bool CInput::ReadVersion(int &nMajorVersion, int &nMinorVersion) {
  ...
  printf("\nSelect protocol version 1.0 - %d.%d (Default %d.%d): ",
         nMaxMajorVersion, nMaxMinorVersion, nMaxMajorVersion,
         nMaxMinorVersion);
  gets_s(pVerStr, 254);
  printf("\n");
...
bool CInput::ReadOperation(EOperation &eOperation) {
  ...
  if (mnMajorVersion > 1 || mnMinorVersion > 6) {
    printf(
        "7 : View Settings (General, 3D, 6DOF, GazeVector, Analog, Force & "
        "Image)\n");
  }
  if (mnMajorVersion > 1 || mnMinorVersion > 6) {
...
bool CInput::ReadDataComponents(unsigned int &nComponentType,
                                char *selectedAnalogChannels,
                                int selectedAnalogChannelsLen) {
...
bool CInput::ReadDataTest(bool bLogSelection, bool &bStreamTCP,
                          bool &bStreamUDP, bool &bLogToFile,
                          bool &bOnlyTimeAndFrameNumber,
                          unsigned short &nUDPPort, char *tUDPAddress,
                          int nAddressLen) {
  int nSelection;
...
void CInput::ReadSystemSettings(unsigned int &nCaptureFrequency,
                                float &fCaptureTime, bool &bExternalTrigger,
                                bool &trigNO, bool &trigNC,
                                bool &trigSoftware) {
...
void CInput::ReadProcessingActionsSettings(
  ...
  const _TCHAR *processings[3] = {"Processing Actions",
                                  "Real-time Processing Actions",
                                  "Reprocessing Actions"};
  CRTProtocol::EProcessingActions *processingActions[3] = {
      &eProcessingActions, &eRtProcessingActions, &eReprocessingActions};
  ...
  for (auto i = 0; i < actionsCount; i++) {
    ...
    {
      *processingActions[i] = (CRTProtocol::EProcessingActions)(
          *processingActions[i] + CRTProtocol::ProcessingTracking2D);
    } else if (nTracking == '3') {
      *processingActions[i] = (CRTProtocol::EProcessingActions)(
          *processingActions[i] + CRTProtocol::ProcessingTracking3D);
    } else {
      *processingActions[i] = (CRTProtocol::EProcessingActions)(
          *processingActions[i] + CRTProtocol::ProcessingNone);
    }
    ...
    if (i != 1)  // Not RtProcessingSettings
    {
      ...
      if (ReadYesNo(true)) {
        *processingActions[i] = (CRTProtocol::EProcessingActions)(
            *processingActions[i] + CRTProtocol::ProcessingTwinSystemMerge);
      }

      printf("Spline Fill (y/n)? ");
      if (ReadYesNo(true)) {
        *processingActions[i] = (CRTProtocol::EProcessingActions)(
            *processingActions[i] + CRTProtocol::ProcessingSplineFill);
      }
    }
    ...
    if (ReadYesNo(true)) {
      *processingActions[i] = (CRTProtocol::EProcessingActions)(
          *processingActions[i] + CRTProtocol::ProcessingAIM);
    }

    printf("6DOF Tracking (y/n)? ");
    if (ReadYesNo(true)) {
      *processingActions[i] = (CRTProtocol::EProcessingActions)(
          *processingActions[i] + CRTProtocol::Processing6DOFTracking);
    }

    printf("Force (y/n)? ");
    if (ReadYesNo(true)) {
      *processingActions[i] = (CRTProtocol::EProcessingActions)(
          *processingActions[i] + CRTProtocol::ProcessingForceData);
    }

    printf("Gaze Vector (y/n)? ");
    if (ReadYesNo(true)) {
      *processingActions[i] = (CRTProtocol::EProcessingActions)(
          *processingActions[i] + CRTProtocol::ProcessingGazeVector);
    }
    ...
    if (i != 1)  // Not RtProcessingSettings
    {
      ...
      if (ReadYesNo(true)) {
        *processingActions[i] = (CRTProtocol::EProcessingActions)(
            *processingActions[i] + CRTProtocol::ProcessingExportTSV);
      }

      printf("Export C3D (y/n)? ");
      if (ReadYesNo(true)) {
        *processingActions[i] = (CRTProtocol::EProcessingActions)(
            *processingActions[i] + CRTProtocol::ProcessingExportC3D);
      }

      printf("Export MATLAB File (y/n)? ");
      if (ReadYesNo(true)) {
        *processingActions[i] = (CRTProtocol::EProcessingActions)(
            *processingActions[i] + CRTProtocol::ProcessingExportMatlabFile);
      }

      printf("Export AVI File (y/n)? ");
      if (ReadYesNo(true)) {
        *processingActions[i] = (CRTProtocol::EProcessingActions)(
            *processingActions[i] + CRTProtocol::ProcessingExportAviFile);
      }
    }
    printf("\n");
  }
...
void CInput::ReadExtTimeBaseSettings(
    bool &bEnabled, int &nSignalSource, bool &bSignalModePeriodic,
    unsigned int &nMultiplier, unsigned int &nDivisor,
    unsigned int &nFrequencyTolerance, float &fNominalFrequency,
    bool &bNegativeEdge, unsigned int &nSignalShutterDelay,
  ...
  if (bEnabled) {
    ...
    if ((nSignalSource == 0 || nSignalSource == 1 || nSignalSource == 2 ||
         nSignalSource == 3) &&
        bSignalModePeriodic) {
...
void CInput::ReadCameraSettings(unsigned int &nCameraId, int &nMode,
                                int &nVideoMode, unsigned int &nVideoFrequency,
                                float &fVideoExposure, float &fVideoFlashTime,
                                float &fMarkerExposure, float &fMarkerThreshold,
                                int &nRotation) {
  printf("\nEnter Camera ID : ");
  nCameraId = ReadInt(1);
...
void CInput::ReadCameraSyncOutSettings(unsigned int &nCameraId, int &portNumber,
                                       int &nSyncOutMode,
  ...
  if (portNumber > 0 && portNumber < 3) {
    ...
    if (nSyncOutMode >= 2 && nSyncOutMode <= 4) {
      printf("Enter %s : ",
             nSyncOutMode == 2
                 ? "Multiplier"
                 : (nSyncOutMode == 3 ? "Divisor"
                                      : "Camera Independent Frequency"));
...
void CInput::ReadImageSettings(unsigned int &nCameraId, bool &bEnable,
                               int &nFormat, unsigned int &nWidth,
                               unsigned int &nHeight, float &fLeftCrop,
                               float &fTopCrop, float &fRightCrop,
                               float &fBottomCrop) {
  int c;
...
void CInput::ReadForceSettings(unsigned int &nForcePlateIndex,
                               float afCorner[4][3]) {
