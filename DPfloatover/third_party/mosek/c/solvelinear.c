...
MSKrescodee put_a(MSKtask_t task, double *aval, MSKidxt *asub, MSKidxt *ptrb,
                  MSKidxt *ptre, int numvar, MSKidxt *basis)
=======
...
<<<<<<< HEAD
  ...
  if (skx == NULL && numvar) r = MSK_RES_ERR_SPACE;

  skc = (MSKstakeye *)calloc(numvar, sizeof(MSKstakeye));
  if (skc == NULL && numvar) r = MSK_RES_ERR_SPACE;
...
<<<<<<< HEAD
  ...
  if (r == MSK_RES_OK) r = MSK_appendcons(task, numvar);
=======
  if (r == MSK_RES_OK)
...
<<<<<<< HEAD
  if (r == MSK_RES_OK) r = MSK_deletesolution(task, MSK_SOL_BAS);
=======
  if (r == MSK_RES_OK)
...
>>>>>>> real time control
  ...
  for (i = 0; i < numvar && r == MSK_RES_OK; ++i)
<<<<<<< HEAD
    r = MSK_putsolutioni(task, MSK_ACC_VAR, i, MSK_SOL_BAS, skx[i], 0.0, 0.0,
                         0.0, 0.0);

  for (i = 0; i < numvar && r == MSK_RES_OK; ++i)
    r = MSK_putsolutioni(task, MSK_ACC_CON, i, MSK_SOL_BAS, skc[i], 0.0, 0.0,
                         0.0, 0.0);

  if (r == MSK_RES_OK) r = MSK_initbasissolve(task, basis);
...
int main(int argc, const char *argv[]) {
  ...
  if (r == MSK_RES_OK) r = MSK_makeenv(&env, NULL);

  if (r == MSK_RES_OK) r = MSK_makeemptytask(env, &task);

  if (r == MSK_RES_OK)
    MSK_linkfunctotaskstream(task, MSK_STREAM_LOG, NULL, printstr);

  basis = (MSKidxt *)calloc(numcon, sizeof(MSKidxt));
  if (basis == NULL && numvar) r = MSK_RES_ERR_SPACE;

  /* Put A matrix and factor A.
     Call this function only once for a given task. */
  if (r == MSK_RES_OK) r = put_a(task, aval, asub, ptrb, ptre, numvar, basis);
=======
int main(int argc, const char *argv[])
{
  MSKenv_t  env;
...
<<<<<<< HEAD
  if (r == MSK_RES_OK) r = MSK_solvewithbasis(task, 0, &nz, bsub, b);
