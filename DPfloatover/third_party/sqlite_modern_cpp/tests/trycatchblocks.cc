...
public:
  ...
  void LogRequest(const string &username, const string &ip,
                  const string &request) {
    try {
      auto timestamp = std::to_string(time(nullptr));

      db << "create table if not exists log_request ("
  ...
  bool TestData(void) {
    try {
      ...
      db << "select username, timestamp, ip, request from log_request where "
            "username = ?"
         << "test" >>
