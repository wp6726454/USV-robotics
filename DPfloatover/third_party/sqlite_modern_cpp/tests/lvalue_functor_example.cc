...
template <typename Target, typename... AttrTypes> struct builder {
...
struct user {
  ...
  user(int age, string name, double weight)
  ...
  static std::vector<user> all(sqlite::database &db) {
    ...
    db << "SELECT * FROM user;" >> person_builder;
...
int main() {
  ...
  try {
    ...
    db << "insert into user (age,name,weight) values (?,?,?);" << 20
       << u"chandler" << 83.25;
    db << "insert into user (age,name,weight) values (?,?,?);" << 21
       << u"monika" << 86.25;
    db << "insert into user (age,name,weight) values (?,?,?);" << 22 << u"ross"
       << 88.25;
