...
struct is_callable<
    Functor(Arguments...),
    void_t<decltype(std::declval<Functor>()(std::declval<Arguments>()...))>>
    : std::true_type {};
...
typename std::enable_if<
    !detail::is_callable<Handler(const sqlite_exception &)>::value>::type
...
error_log(Handler &&... handler) {
  return error_log(
      detail::FunctorOverload<
          detail::WrapIntoFunctor<typename std::decay<Handler>::type>...>(
...
typename std::enable_if<
    !detail::is_callable<Handler(const sqlite_exception &)>::value>::type
error_log(Handler &&handler) {
  return error_log(std::forward<Handler>(handler),
                   [](const sqlite_exception &) {});
}
template <class Handler>
typename std::enable_if<
    detail::is_callable<Handler(const sqlite_exception &)>::value>::type
error_log(Handler &&handler) {
  auto ptr = detail::make_shared_inferred(
      [handler = std::forward<Handler>(handler)](int error_code,
                                                 const char *errstr) mutable {
...
#undef SQLITE_MODERN_CPP_ERROR_CODE
  ...
  sqlite3_config(SQLITE_CONFIG_LOG,
                 (void (*)(void *, int, const char *))[](
                     void *functor, int error_code, const char *errstr) {
                   (*static_cast<decltype(ptr.get())>(functor))(error_code,
                                                                errstr);
                 },
                 ptr.get());
