...
public:
  ...
  database_binder(database_binder &&other)
      : _db(std::move(other._db)), _stmt(std::move(other._stmt)),
...
private:
  ...
  friend database_binder &
  operator<<(database_binder &db, const boost::optional<BoostOptionalT> &val);
  template <typename BoostOptionalT>
  friend void get_col_from_db(database_binder &db, int inx,
...
inline database_binder &operator<<(database_binder &db, const int &val) {
  int hresult;
  if ((hresult = sqlite3_bind_int(db._stmt.get(), db._next_index(), val)) !=
      SQLITE_OK) {
...
inline database_binder &operator<<(database_binder &db,
                                   const sqlite_int64 &val) {
  int hresult;
  if ((hresult = sqlite3_bind_int64(db._stmt.get(), db._next_index(), val)) !=
      SQLITE_OK) {
    errors::throw_sqlite_error(hresult, db.sql());
  }
...
inline database_binder &operator<<(database_binder &db, const float &val) {
  int hresult;
  if ((hresult = sqlite3_bind_double(db._stmt.get(), db._next_index(),
                                     double(val))) != SQLITE_OK) {
    errors::throw_sqlite_error(hresult, db.sql());
  }
...
inline database_binder &operator<<(database_binder &db, const double &val) {
  int hresult;
  if ((hresult = sqlite3_bind_double(db._stmt.get(), db._next_index(), val)) !=
      SQLITE_OK) {
    errors::throw_sqlite_error(hresult, db.sql());
  }
...
inline database_binder &operator<<(database_binder &db,
  ...
  int hresult;
  if ((hresult = sqlite3_bind_blob(db._stmt.get(), db._next_index(), buf, bytes,
...
inline database_binder &operator<<(database_binder &db, std::nullptr_t) {
  int hresult;
  if ((hresult = sqlite3_bind_null(db._stmt.get(), db._next_index())) !=
      SQLITE_OK) {
    errors::throw_sqlite_error(hresult, db.sql());
  }
  return db;
...
inline database_binder &operator<<(database_binder &db,
                                   const std::string &txt) {
  int hresult;
  if ((hresult = sqlite3_bind_text(db._stmt.get(), db._next_index(), txt.data(),
...
inline database_binder &operator<<(database_binder &db,
                                   const std::u16string &txt) {
  int hresult;
  if ((hresult = sqlite3_bind_text16(db._stmt.get(), db._next_index(),
                                     txt.data(), -1, SQLITE_TRANSIENT)) !=
      SQLITE_OK) {
    errors::throw_sqlite_error(hresult, db.sql());
  }
